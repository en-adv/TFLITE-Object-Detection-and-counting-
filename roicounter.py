...
def line_intersection(line1_start, line1_end, line2_start, line2_end):
    """
    Check if two lines (defined by their start and end points) intersect.
...
# Define ROI line coordinates
x1, y1 = 100, 200
x2, y2 = 500, 200

# Initialize object count across ROI line
object_count_roi = {}
...
while True:
    ...
    for i in range(len(scores)):
        if ((scores[i] > min_conf_threshold) and (scores[i] <= 1.0)):
            ...
            # Get object label and count number of occurrences
            object_name = labels[int(classes[i])]
            if object_name in object_count_frame:
                object_count_frame[object_name] += 1
            else:
                object_count_frame[object_name] = 1

            # Draw label and count above the box
            label = '{}: {}'.format(object_name, object_count_frame[object_name])
            ...
            # Check if bounding box crosses the ROI line
            roi_line_start = (x1, y1)
            roi_line_end = (x2, y2)
            bbox_top_left = (xmin, ymin)
            bbox_bottom_right = (xmax, ymax)
            if line_intersection(roi_line_start, roi_line_end, bbox_top_left, bbox_bottom_right):
                if object_name in object_count_roi:
                    object_count_roi[object_name] += 1
                else:
                    object_count_roi[object_name] = 1

    # Update object count dictionary with the current frame counts
    for object_name, count in object_count_frame.items():
        if object_name in object_count:
            object_count[object_name] = max(object_count[object_name], count)
        else:
    ...
    for object_name, count in object_count.items():
        cv2.putText(frame, '{}: {}'.format(object_name, count), (10, y), cv2.FONT_HERSHEY_SIMPLEX, 0.75,
                    (255, 255, 0), 2)
        y += 30

    # Display object count across ROI line
    cv2.putText(frame, 'Objects Crossing ROI: {}'.format(sum(object_count_roi.values())), (10, y + 30),
