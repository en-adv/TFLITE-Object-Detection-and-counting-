...
broker_port = 15907
broker_username = 'ibxznyfv'
broker_password = 'CeTN0fS6rPFf'
topic = 'viewers'
...
while True:
    ...
    for object_name in object_count:
        if object_name in object_count_frame:
            object_count[object_name] = object_count_frame[object_name]
        else:
            object_count[object_name] = 0

    # Add newly detected objects to the object count dictionary
    for object_name in object_count_frame:
    ...
    for object_name, count in object_count.items():
        cv2.putText(frame, '{}: {}'.format(object_name, count), (10, y), cv2.FONT_HERSHEY_SIMPLEX, 0.75,
                    (72,56, 100), 2)
        y += 30
        # Publish the new total face count to MQTT
        # Publish the new total face count to MQTT
        topic = "viewers/{}".format(object_name)  # Modify the topic prefix as per your requirement
